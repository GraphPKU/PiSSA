...
def animate_contour(
    ...
    ax.plot(true_optim_point[0], true_optim_point[1], "bx", label="Target Local Minimum")
    ...
    (full_pathline,) = ax.plot(full_w1s, full_w2s, color="b", lw=1, label="Full FT")
    (full_point,) = ax.plot(full_W0[0], full_W0[1], "bo")
    (lora_pathline,) = ax.plot(lora_w1s, lora_w2s, color="g", lw=1, label="LoRA")
    (lora_point,) = ax.plot(lora_W0[0], lora_W0[1], "go")
    (pissa_pathline,) = ax.plot(pissa_w1s, pissa_w2s, color="r", lw=1, label="PiSSA")
    (pissa_point,) = ax.plot(pissa_W0[0], pissa_W0[1], "ro")
    ...
    def animate(i):
        ...
        if i % 20 == 19:
            ax.plot(full_W[0], full_W[1], "b+")
            ax.plot(lora_W[0], lora_W[1], "g+")
            ax.plot(pissa_W[0], pissa_W[1], "r+")
